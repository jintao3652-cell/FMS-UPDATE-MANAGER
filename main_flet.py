        ...
        try:
            ...
            async def try_reuse_saved_token(show_busy: bool, close_on_success: bool) -> bool:
                ...
                try:
                    ...
                    network_hints = ("无法连接服务器", "网络连接失败", "winerror", "urlopen error", "timed out", "connection")
                    invalid_hints = ("invalid token", "token", "authorization", "unauthorized", "missing authorization", "已失效", "过期")
                    if any(hint in lowered for hint in network_hints):
                        result_text.value = ""
                        err_text.value = _("无法连接服务器，请检查网络后重试。")
                    elif any(hint in lowered for hint in invalid_hints):
                        clear_saved_backup_power_token()
                        result_text.value = ""
                        err_text.value = _("登录过期，请重新登录。")
                    else:
                        result_text.value = ""
                        err_text.value = _("登录过期，请重新登录。")
                    set_backup_power_login_valid(False)
            ...
            def open_register_page(_evt=None) -> None:
                try:
                    webbrowser.open("http://fms.cnrpg.top/register", new=2)
                except Exception as exc:
                    snack(_("打开注册页面失败: {exc}", exc=exc))

            register_link = ft.Row(
                spacing=4,
                controls=[
                    ft.Text(_("还没有账号？"), size=fs(11), color=colors["text_sub"]),
                    ft.TextButton(
                        _("点击注册"),
                        on_click=open_register_page,
