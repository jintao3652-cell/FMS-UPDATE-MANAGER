...
- `assets/`：图标等静态资源
- `installer/`：安装器相关文件
- `deploy/backup_auth/`：后端认证与注册管理服务（FastAPI + MySQL + Docker Compose）
...
- `FMS_APP_VERSION`：当前应用版本号（建议语义化版本，如 `1.0.0`）
...
```bash
cd deploy/backup_auth
docker compose up -d --build
```

如果需要随服务器开机自动启动，请参考：

- `deploy/backup_auth/AUTO_START.md`
- `deploy/backup_auth/fms-backup-auth.service`
