  ...
  <div class="grid">
    <aside class="panel side">
      ...
      <div class="nav">
        ...
        <button data-page="rate_limits" onclick="switchPage('rate_limits')">限流设置</button>
        <button data-page="register_policy" onclick="switchPage('register_policy')">注册策略</button>
        <button data-page="invites" onclick="switchPage('invites')">邀请码</button>
        <button data-page="outbox" onclick="switchPage('outbox')">邮件 Outbox</button>
        ...
        <button data-page="crash_logs" onclick="switchPage('crash_logs')">崩溃日志</button>
        <button data-page="openlist" onclick="switchPage('openlist')">OpenList 期数</button>
      </div>
    </aside>
    <main class="panel main">
      ...
      <!-- Rate limits -->
      <section id="page-rate_limits" class="hidden">
        <div class="section">
          <h3>限流设置</h3>
          <div class="row">
            <div><label>登录失败窗口（秒）</label><input id="rl_login_window" type="number"></div>
            <div><label>登录失败上限</label><input id="rl_login_fail" type="number"></div>
          </div>
          <div class="row">
            <div><label>注册尝试上限</label><input id="rl_reg_attempt" type="number"></div>
            <div><label>注册尝试窗口（秒）</label><input id="rl_reg_attempt_window" type="number"></div>
          </div>
          <div class="row">
            <div><label>每 IP 注册成功上限</label><input id="rl_reg_success" type="number"></div>
            <div><label>每 IP 注册成功窗口（秒）</label><input id="rl_reg_success_window" type="number"></div>
          </div>
          <div class="row">
            <div><label>验证码每 IP 冷却（秒）</label><input id="rl_code_send_cool" type="number"></div>
            <div></div>
          </div>
          <div class="row">
            <div><label>崩溃上报窗口（秒）</label><input id="rl_crash_window" type="number"></div>
            <div><label>崩溃上报上限</label><input id="rl_crash_limit" type="number"></div>
          </div>
          <div style="margin-top:14px" class="flex">
            <button class="btn primary" onclick="saveRateLimits()">保存限流</button>
            <button class="btn" onclick="loadRateLimits()">重新加载</button>
          </div>
          <div class="hint">改动立即生效，无需重启服务。所有限制按 IP 在内存中滑动窗口计算。</div>
          <div id="rl_msg" class="msg hidden"></div>
        </div>
      </section>

      <!-- Register policy -->
      <section id="page-register_policy" class="hidden">
        <div class="section">
          <h3>注册策略</h3>
          <div class="row">
            <div>
              <label>需要邀请码注册</label>
              <select id="rp_invite"><option value="false">否</option><option value="true">是</option></select>
            </div>
            <div>
              <label>新用户需管理员审核</label>
              <select id="rp_approval"><option value="false">否</option><option value="true">是</option></select>
            </div>
          </div>
          <div style="margin-top:14px" class="flex">
            <button class="btn primary" onclick="saveRegisterPolicy()">保存策略</button>
            <button class="btn" onclick="loadRegisterPolicy()">重新加载</button>
          </div>
          <div class="hint">审核开启后，新注册账号 enabled=False，需要在“用户管理”里启用后才能登录。</div>
          <div id="rp_msg" class="msg hidden"></div>
        </div>
        <div class="section" style="margin-top:14px">
          <h3>待审核用户</h3>
          <button class="btn" onclick="loadPendingUsers()">刷新</button>
          <table id="pending_table" style="margin-top:8px">
            <thead><tr><th>ID</th><th>用户名</th><th>邮箱</th><th>注册时间</th><th></th></tr></thead>
            <tbody></tbody>
          </table>
          <div id="pending_msg" class="msg hidden"></div>
        </div>
      </section>

      <!-- Invite codes -->
      <section id="page-invites" class="hidden">
        <div class="section">
          <h3>邀请码</h3>
          <div class="row">
            <div><label>新邀请码（留空自动生成）</label><input id="inv_code"></div>
            <div><label>备注</label><input id="inv_note"></div>
          </div>
          <div class="row">
            <div><label>最大使用次数</label><input id="inv_max" type="number" value="1"></div>
            <div><label>过期时间（ISO 8601，可选）</label><input id="inv_exp" placeholder="2026-12-31T23:59:59Z"></div>
          </div>
          <div style="margin-top:14px" class="flex">
            <button class="btn primary" onclick="createInvite()">生成邀请码</button>
            <button class="btn" onclick="loadInvites()">刷新</button>
          </div>
          <table id="invites_table" style="margin-top:14px">
            <thead><tr><th>ID</th><th>邀请码</th><th>备注</th><th>已用/上限</th><th>启用</th><th>过期时间</th><th>创建者</th><th>创建时间</th><th></th></tr></thead>
            <tbody></tbody>
          </table>
          <div id="invites_msg" class="msg hidden"></div>
        </div>
      </section>

      <!-- Outbox (email retry queue) -->
      <section id="page-outbox" class="hidden">
        <div class="section">
          <h3>邮件 Outbox（重试队列）</h3>
          <div class="flex" style="margin-bottom:10px">
            <div>
              <select id="outbox_status">
            ...
            <div class="grow"></div>
            <button class="btn" onclick="loadOutbox()">刷新</button>
          </div>
          <table id="outbox_table">
            <thead><tr><th>ID</th><th>收件人</th><th>主题</th><th>状态</th><th>尝试</th><th>下次</th><th>错误</th><th>创建</th><th>已发</th><th></th></tr></thead>
            <tbody></tbody>
          </table>
          <div id="outbox_msg" class="msg hidden"></div>
        </div>
      </section>
      ...
      <!-- Crash logs -->
      <section id="page-crash_logs" class="hidden">
        <div class="section">
          <h3>客户端崩溃日志</h3>
          <div class="flex" style="margin-bottom:10px">
            <div><input id="crash_version" placeholder="版本，如 1.0.5"></div>
            <div><input id="crash_exc_type" placeholder="异常类型，如 ValueError"></div>
            <div class="grow"></div>
            <button class="btn" onclick="loadCrashLogs()">刷新</button>
          </div>
          <table id="crash_table">
            <thead><tr><th>ID</th><th>版本</th><th>类型</th><th>消息</th><th>install_id</th><th>IP</th><th>时间</th><th></th></tr></thead>
            <tbody></tbody>
          </table>
          <div id="crash_msg" class="msg hidden"></div>
          <div id="crash_detail" class="section hidden" style="margin-top:14px">
            <h4 style="margin:0 0 8px;color:#8ea3c7">详情</h4>
            <pre id="crash_detail_body" style="white-space:pre-wrap;word-break:break-all;font-size:12px;background:rgba(0,0,0,.3);padding:12px;border-radius:8px;max-height:480px;overflow:auto"></pre>
          </div>
        </div>
      </section>

      <!-- OpenList cycle monitor -->
      <section id="page-openlist" class="hidden">
        <div class="section">
          <h3>OpenList 期数监控</h3>
          <div class="flex" style="margin-bottom:10px">
            <div class="grow"><span class="pill" id="openlist_latest">最新: --</span></div>
            <button class="btn" onclick="loadOpenlistCycles()">刷新</button>
          </div>
          <table id="openlist_cycles_table">
            <thead><tr><th>期数</th><th>修改时间</th><th>操作</th></tr></thead>
            <tbody></tbody>
          </table>
          <div id="openlist_msg" class="msg hidden"></div>
          <div id="openlist_cycle_files" class="section hidden" style="margin-top:14px">
            <h4 style="margin:0 0 8px;color:#8ea3c7">期数 <span id="openlist_cycle_name">--</span> · MSFS 包 (<span id="openlist_cycle_count">0</span>)</h4>
            <table id="openlist_files_table">
...
const REFRESH_KEY = 'fms_admin_refresh';
...
const PAGE_META = {
  ...
  rate_limits: {title:'限流设置', sub:'登录/注册/验证码/崩溃上报限流'},
  register_policy: {title:'注册策略', sub:'邀请码 / 管理员审核'},
  invites: {title:'邀请码', sub:'生成与禁用邀请码'},
  outbox: {title:'邮件 Outbox', sub:'SMTP 发送重试队列'},
  email_logs: {title:'邮件日志', sub:'邮件发送历史记录'},
  audit_logs: {title:'审计日志', sub:'管理员操作与登录记录'},
  crash_logs: {title:'崩溃日志', sub:'客户端上报的异常报告'},
  openlist: {title:'OpenList 期数', sub:'查看可用 AIRAC 期数与 MSFS 包列表'}
};
...
function switchPage(name){
  ...
  else if(name === 'rate_limits') loadRateLimits();
  else if(name === 'register_policy') loadRegisterPolicy();
  else if(name === 'invites') loadInvites();
  else if(name === 'outbox') loadOutbox();
  else if(name === 'email_logs') loadEmailLogs();
  else if(name === 'audit_logs'){ loadAuditLogs(); loadLoginLogs(); }
  else if(name === 'crash_logs') loadCrashLogs();
  else if(name === 'openlist') loadOpenlistCycles();
}
...
async function loadRateLimits(){
  showMsg('rl_msg','',true);
  try{
    const r = await apiFetch('/api/settings/rate_limits');
    if(!r.ok){ showMsg('rl_msg','加载失败 ' + r.status, false); return; }
    const d = await r.json();
    const c = d.rate_limits || {};
    document.getElementById('rl_login_window').value = c.login_window_seconds ?? 300;
    document.getElementById('rl_login_fail').value = c.login_fail_limit ?? 8;
    document.getElementById('rl_reg_attempt').value = c.register_attempt_limit ?? 10;
    document.getElementById('rl_reg_attempt_window').value = c.register_attempt_window_seconds ?? 300;
    document.getElementById('rl_reg_success').value = c.register_per_ip_limit ?? 1;
    document.getElementById('rl_reg_success_window').value = c.register_per_ip_window_seconds ?? 86400;
    document.getElementById('rl_code_send_cool').value = c.code_send_per_ip_cooldown ?? 60;
    document.getElementById('rl_crash_window').value = c.crash_rate_window_seconds ?? 60;
    document.getElementById('rl_crash_limit').value = c.crash_rate_limit ?? 20;
  }catch(e){ showMsg('rl_msg', e.message||String(e), false); }
}

async function saveRateLimits(){
  showMsg('rl_msg','',true);
  const body = {
    login_window_seconds: Number(document.getElementById('rl_login_window').value)||300,
    login_fail_limit: Number(document.getElementById('rl_login_fail').value)||8,
    register_attempt_limit: Number(document.getElementById('rl_reg_attempt').value)||10,
    register_attempt_window_seconds: Number(document.getElementById('rl_reg_attempt_window').value)||300,
    register_per_ip_limit: Number(document.getElementById('rl_reg_success').value)||1,
    register_per_ip_window_seconds: Number(document.getElementById('rl_reg_success_window').value)||86400,
    code_send_per_ip_cooldown: Number(document.getElementById('rl_code_send_cool').value)||60,
    crash_rate_window_seconds: Number(document.getElementById('rl_crash_window').value)||60,
    crash_rate_limit: Number(document.getElementById('rl_crash_limit').value)||20,
  };
  try{
    const r = await apiFetch('/api/settings/rate_limits', {method:'PUT', body: JSON.stringify(body)});
    const d = await r.json();
    if(!r.ok){ showMsg('rl_msg', d.detail || '保存失败', false); return; }
    showMsg('rl_msg', '已保存', true);
    loadRateLimits();
  }catch(e){ showMsg('rl_msg', e.message||String(e), false); }
}

async function loadOutbox(){
  showMsg('outbox_msg','',true);
  const params = new URLSearchParams();
  const st = document.getElementById('outbox_status').value;
  if(st) params.set('status_filter', st);
  params.set('limit', '100');
  try{
    const r = await apiFetch('/api/admin/outbox?' + params.toString());
    if(!r.ok){ showMsg('outbox_msg','加载失败 ' + r.status, false); return; }
    const d = await r.json();
    const tb = document.querySelector('#outbox_table tbody');
    tb.innerHTML = '';
    for(const it of (d.items||[])){
      const canRetry = it.status !== 'sent';
      const tr = document.createElement('tr');
      tr.innerHTML =
        '<td>'+it.id+'</td>'+
        '<td>'+escapeHtml(it.recipient)+'</td>'+
        ...
        '<td>'+it.attempts+'</td>'+
        '<td>'+escapeHtml(it.next_attempt_at||'')+'</td>'+
        '<td style="max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+escapeHtml(it.last_error||'')+'">'+escapeHtml(it.last_error||'')+'</td>'+
        '<td>'+escapeHtml(it.created_at||'')+'</td>'+
        '<td>'+escapeHtml(it.sent_at||'')+'</td>'+
        '<td>'+(canRetry?'<button class="btn ghost" onclick="retryOutbox('+it.id+')">重试</button>':'')+'</td>';
      tb.appendChild(tr);
    }
  }catch(e){ showMsg('outbox_msg', e.message||String(e), false); }
}

async function retryOutbox(id){
  try{
    const r = await apiFetch('/api/admin/outbox/'+id+'/retry', {method:'POST'});
    const d = await r.json();
    if(!r.ok){ showMsg('outbox_msg', d.detail || '重试失败', false); return; }
    showMsg('outbox_msg', '已重新入队，等待 worker 处理', true);
    loadOutbox();
  }catch(e){ showMsg('outbox_msg', e.message||String(e), false); }
}
...
async function loadCrashLogs(){
  showMsg('crash_msg','',true);
  document.getElementById('crash_detail').classList.add('hidden');
  const params = new URLSearchParams();
  const v = document.getElementById('crash_version').value.trim();
  const t = document.getElementById('crash_exc_type').value.trim();
  if(v) params.set('version', v);
  if(t) params.set('exc_type', t);
  params.set('limit', '100');
  try{
    const r = await apiFetch('/api/logs/crash?' + params.toString());
    if(!r.ok){ showMsg('crash_msg','加载失败 ' + r.status, false); return; }
    const d = await r.json();
    const tb = document.querySelector('#crash_table tbody');
    tb.innerHTML = '';
    for(const item of (d.items||[])){
      const tr = document.createElement('tr');
      tr.innerHTML = '<td>'+item.id+'</td><td>'+item.version+'</td><td>'+item.exc_type+'</td><td style="max-width:320px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+escapeHtml(item.exc_msg||'')+'</td><td>'+item.install_id.slice(0,8)+'</td><td>'+item.ip+'</td><td>'+item.created_at+'</td><td><button class="btn ghost" onclick="viewCrash('+item.id+')">查看</button></td>';
      tb.appendChild(tr);
    }
  }catch(e){ showMsg('crash_msg', e.message||String(e), false); }
}

async function viewCrash(id){
  try{
    const r = await apiFetch('/api/logs/crash/'+id);
    if(!r.ok){ showMsg('crash_msg','加载详情失败', false); return; }
    const d = await r.json();
    const it = d.item;
    const body = '== '+it.created_at+' ==\n'
      + 'app: '+it.app+' v'+it.version+'\n'
      + 'install_id: '+it.install_id+'\n'
      + 'platform: '+it.platform+' / python '+it.python+'\n'
      + 'kind: '+it.kind+'\n'
      + 'ip: '+it.ip+'\n'
      + 'exc: '+it.exc_type+': '+it.exc_msg+'\n'
      + 'extra: '+it.extra+'\n'
      + '--- traceback ---\n'+it.traceback;
    document.getElementById('crash_detail_body').textContent = body;
    document.getElementById('crash_detail').classList.remove('hidden');
  }catch(e){ showMsg('crash_msg', e.message||String(e), false); }
}
...
async function loadOpenlistCycles(){
  showMsg('openlist_msg','',true);
  document.getElementById('openlist_cycle_files').classList.add('hidden');
  try{
    const r = await apiFetch('/api/openlist/cycles');
    if(!r.ok){
      const t = await r.text();
      showMsg('openlist_msg','加载失败: ' + t.slice(0,200), false);
      return;
    }
    const d = await r.json();
    document.getElementById('openlist_latest').textContent = '最新: ' + (d.latest || '无');
    const tb = document.querySelector('#openlist_cycles_table tbody');
    tb.innerHTML = '';
    for(const c of (d.items||[])){
      const tr = document.createElement('tr');
      tr.innerHTML = '<td>'+c.cycle_id+'</td><td>'+(c.modified||'')+'</td><td><button class="btn ghost" onclick="loadOpenlistFiles(\''+c.cycle_id+'\')">查看 MSFS 包</button></td>';
      tb.appendChild(tr);
    }
  }catch(e){ showMsg('openlist_msg', e.message||String(e), false); }
}

async function loadOpenlistFiles(cycle){
  showMsg('openlist_msg','',true);
  try{
    const r = await apiFetch('/api/openlist/cycles/' + encodeURIComponent(cycle) + '/msfs');
    if(!r.ok){
      const t = await r.text();
      showMsg('openlist_msg','加载失败: ' + t.slice(0,200), false);
      return;
    ...
    const d = await r.json();
    document.getElementById('openlist_cycle_name').textContent = cycle;
    document.getElementById('openlist_cycle_count').textContent = d.count;
    const tb = document.querySelector('#openlist_files_table tbody');
    tb.innerHTML = '';
    for(const f of (d.items||[])){
      const tr = document.createElement('tr');
      tr.innerHTML = '<td>'+escapeHtml(f.name)+'</td><td>'+fmtBytes(f.size)+'</td><td>'+(f.modified||'')+'</td>';
      tb.appendChild(tr);
    }
    document.getElementById('openlist_cycle_files').classList.remove('hidden');
  }catch(e){ showMsg('openlist_msg', e.message||String(e), false); }
}
...
async function loadRegisterPolicy(){
  showMsg('rp_msg','',true);
  try{
    const r = await apiFetch('/api/settings/register_policy');
    if(!r.ok){ showMsg('rp_msg','加载失败 ' + r.status, false); return; }
    const d = await r.json();
    const p = d.policy || {};
    document.getElementById('rp_invite').value = p.require_invite_code ? 'true' : 'false';
    document.getElementById('rp_approval').value = p.require_admin_approval ? 'true' : 'false';
    loadPendingUsers();
  }catch(e){ showMsg('rp_msg', e.message||String(e), false); }
}

async function saveRegisterPolicy(){
  showMsg('rp_msg','',true);
  const body = {
    require_invite_code: document.getElementById('rp_invite').value === 'true',
    require_admin_approval: document.getElementById('rp_approval').value === 'true',
  };
  try{
    const r = await apiFetch('/api/settings/register_policy', {method:'PUT', body: JSON.stringify(body)});
    const d = await r.json();
    if(!r.ok){ showMsg('rp_msg', d.detail || '保存失败', false); return; }
    showMsg('rp_msg', '已保存', true);
  }catch(e){ showMsg('rp_msg', e.message||String(e), false); }
}

async function loadPendingUsers(){
  showMsg('pending_msg','',true);
  try{
    const r = await apiFetch('/api/admin/pending_users');
    if(!r.ok){ showMsg('pending_msg','加载失败 ' + r.status, false); return; }
    const d = await r.json();
    const tb = document.querySelector('#pending_table tbody');
    tb.innerHTML = '';
...
async function loadInvites(){
  showMsg('invites_msg','',true);
  try{
    const r = await apiFetch('/api/admin/invites');
    if(!r.ok){ showMsg('invites_msg','加载失败 ' + r.status, false); return; }
    const d = await r.json();
    const tb = document.querySelector('#invites_table tbody');
    tb.innerHTML = '';
    for(const it of (d.items||[])){
      const tr = document.createElement('tr');
      tr.innerHTML =
        ...
        '<td>'+escapeHtml(it.expires_at||'')+'</td>'+
        '<td>'+escapeHtml(it.created_by||'')+'</td>'+
        '<td>'+escapeHtml(it.created_at||'')+'</td>'+
        '<td>'+
          '<button class="btn" onclick="toggleInvite('+it.id+','+(!it.enabled)+')">'+(it.enabled?'禁用':'启用')+'</button> '+
          '<button class="btn ghost" onclick="deleteInvite('+it.id+')">删除</button>'+
        '</td>';
      tb.appendChild(tr);
    }
  }catch(e){ showMsg('invites_msg', e.message||String(e), false); }
}
...
async function createInvite(){
  ...
  const exp = document.getElementById('inv_exp').value.trim();
  if(exp) body.expires_at = exp;
  try{
    const r = await apiFetch('/api/admin/invites', {method:'POST', body: JSON.stringify(body)});
    const d = await r.json();
    if(!r.ok){ showMsg('invites_msg', d.detail || '创建失败', false); return; }
    showMsg('invites_msg', '已生成邀请码: ' + (d.code || ''), true);
...
async function toggleInvite(id, enabled){
  try{
    const r = await apiFetch('/api/admin/invites/' + id, {method:'PATCH', body: JSON.stringify({enabled})});
    if(!r.ok){ showMsg('invites_msg','切换失败', false); return; }
    loadInvites();
  }catch(e){ showMsg('invites_msg', e.message||String(e), false); }
}
...
async function deleteInvite(id){
  ...
  try{
    const r = await apiFetch('/api/admin/invites/' + id, {method:'DELETE'});
    if(!r.ok){ showMsg('invites_msg','删除失败', false); return; }
    loadInvites();
  }catch(e){ showMsg('invites_msg', e.message||String(e), false); }
}
</script>
