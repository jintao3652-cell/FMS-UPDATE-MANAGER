...
# Rules (deliberately gentle — UX matters more than zxcvbn here):
#   * length >= 8
#   * <= 256 (pydantic also enforces)
#   * not in a tiny built-in blocklist
...
def password_strength_problems(pw: str) -> list[str]:
    """Returns a list of human-readable Chinese problem messages.
...
def assert_password_strong(pw: str) -> None:
    """Raises ValueError with a single combined message if pw fails policy."""
